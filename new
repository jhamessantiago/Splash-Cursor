...
function getWebGLContext(e) {
    ...
    const n = !!t;
    let i, o;
    return n || (t = e.getContext("webgl", r) || e.getContext("experimental-webgl", r)), n ? (t.getExtension("EXT_color_buffer_float"), o = t.getExtension("OES_texture_float_linear")) : (i = t.getExtension("OES_texture_half_float"), o = t.getExtension("OES_texture_half_float_linear")), t.clearColor(1, 1, 1, 1), {
        gl: t,
        ext: {
            formatRGBA: n ? t.RGBA16F : t.RGBA,
            formatRG: n ? t.RG16F : t.RGBA,
            formatR: n ? t.R16F : t.RGBA,
            halfFloatTexType: n ? t.HALF_FLOAT : i.HALF_FLOAT_OES,
...
function captureScreenshot() {
    let e = getResolution(config.CAPTURE_RESOLUTION),
        r = createFBO(e.width, e.height, ext.formatRGBA.internalFormat, ext.formatRGBA.format, ext.halfFloatTexType, gl.NEAREST);
    render(r);
...
function normalizeTexture(e, r, t) {
    ...
    for (let o = t - 1; o >= 0; o--)
        for (let t = 0; t < r; t++) {
            let a = o * r * 4 + 4 * t;
            n[a + 0] = 255 * clamp01(e[i + 0]), n[a + 1] = 255 * clamp01(e[i + 1]), n[a + 2] = 255 * clamp01(e[i + 2]), n[a + 3] = 255 * clamp01(e[i + 3]), i += 4
        }
    return n
}
...
function compileShader(e, r, t) {
    ...
    return gl.shaderSource(n, r), gl.compileShader(n), gl.getShaderParameter(n, gl.COMPILE_STATUS) || console.trace(gl.getShaderInfoLog(n)), n
}
...
const baseVertexShader = compileShader(gl.VERTEX_SHADER, "\n    precision highp float;\n\n    attribute vec2 aPosition;\n    varying vec2 vUv;\n    varying vec2 vL;\n    varying vec2 vR;\n    varying vec2 vT;\n    varying vec2 vB;\n    uniform vec2 texelSize;\n\n    void main () {\n        vUv = aPosition * 0.5 + 0.5;\n        vL = vUv - vec2(texelSize.x, 0.0);\n        vR = vUv + vec2(texelSize.x, 0.0);\n        vT = vUv + vec2(0.0, texelSize.y);\n        vB = vUv - vec2(0.0, texelSize.y);\n        gl_Position = vec4(aPosition, 0.0, 1.0);\n    }\n"),
    ...
    blit = (gl.bindBuffer(gl.ARRAY_BUFFER, gl.createBuffer()), gl.bufferData(gl.ARRAY_BUFFER, new Float32Array([-1, -1, -1, 1, 1, 1, 1, -1]), gl.STATIC_DRAW), gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, gl.createBuffer()), gl.bufferData(gl.ELEMENT_ARRAY_BUFFER, new Uint16Array([0, 1, 2, 0, 2, 3]), gl.STATIC_DRAW), gl.vertexAttribPointer(0, 2, gl.FLOAT, !1, 0, 0), gl.enableVertexAttribArray(0), (e, r = !1) => {
        null == e ? (gl.viewport(0, 0, gl.drawingBufferWidth, gl.drawingBufferHeight), gl.bindFramebuffer(gl.FRAMEBUFFER, null)) : (gl.viewport(0, 0, e.width, e.height), gl.bindFramebuffer(gl.FRAMEBUFFER, e.fbo)), r && (gl.clearColor(1, 1, 1, 1), gl.clear(gl.COLOR_BUFFER_BIT)), gl.drawElements(gl.TRIANGLES, 6, gl.UNSIGNED_SHORT, 0)
    });
...
let dye, velocity, divergence, curl, pressure, bloom, sunrays, sunraysTemp, bloomFramebuffers = [],
...
function initBloomFramebuffers() {
    ...
    bloom = createFBO(e.width, e.height, t.internalFormat, t.format, r, n), bloomFramebuffers.length = 0;
...
function createFBO(e, r, t, n, i, o) {
    ...
    gl.bindTexture(gl.TEXTURE_2D, a), gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, o), gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, o), gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE), gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE), gl.texImage2D(gl.TEXTURE_2D, 0, t, e, r, 0, n, i, null);
...
updateKeywords(), initFramebuffers(), multipleSplats(parseInt(20 * Math.random()) + 5);
...
function updateColors(e) {
    config.COLORFUL && (colorUpdateTimer += e * config.COLOR_UPDATE_SPEED, colorUpdateTimer >= 1 && (colorUpdateTimer = wrap(colorUpdateTimer, 0, 1), pointers.forEach((e => {
...
function applyBloom(e, r) {
    ...
    let n = config.BLOOM_THRESHOLD * config.BLOOM_SOFT_KNEE + 1e-4,
        ...
        o = 2 * n,
        a = .25 / n;
    gl.uniform3f(bloomPrefilterProgram.uniforms.curve, i, o, a), gl.uniform1f(bloomPrefilterProgram.uniforms.threshold, config.BLOOM_THRESHOLD), gl.uniform1i(bloomPrefilterProgram.uniforms.uTexture, e.attach(0)), blit(t), bloomBlurProgram.bind();
    ...
    for (let e = bloomFramebuffers.length - 2; e >= 0; e--) {
        let r = bloomFramebuffers[e];
        gl.uniform2f(bloomBlurProgram.uniforms.texelSize, t.texelSizeX, t.texelSizeY), gl.uniform1i(bloomBlurProgram.uniforms.uTexture, t.attach(0)), gl.viewport(0, 0, r.width, r.height), blit(r), t = r
    }
...
function applySunrays(e, r, t) {
    gl.disable(gl.BLEND), sunraysMaskProgram.bind(), gl.uniform1i(sunraysMaskProgram.uniforms.uTexture, e.attach(0)), blit(r), sunraysProgram.bind(), gl.uniform1f(sunraysProgram.uniforms.weight, config.SUNRAYS_WEIGHT), gl.uniform1i(sunraysProgram.uniforms.uTexture, r.attach(0)), blit(t)
}

function blur(e, r, t) {
    blurProgram.bind();
    for (let n = 0; n < t; n++) gl.uniform2f(blurProgram.uniforms.texelSize, e.texelSizeX, 0), gl.uniform1i(blurProgram.uniforms.uTexture, e.attach(0)), blit(r), gl.uniform2f(blurProgram.uniforms.texelSize, 0, e.texelSizeY), gl.uniform1i(blurProgram.uniforms.uTexture, r.attach(0)), blit(e)
}
...
function multipleSplats(e) {
    for (let r = 0; r < e; r++) {
        ...
        splat(Math.random(), Math.random(), 1e3 * (Math.random() - .5), 1e3 * (Math.random() - .5), e)
    }
}

function splat(e, r, t, n, i) {
    splatProgram.bind(), gl.uniform1i(splatProgram.uniforms.uTarget, velocity.read.attach(0)), gl.uniform1f(splatProgram.uniforms.aspectRatio, canvas.width / canvas.height), window.innerWidth <= 768 ? gl.uniform2f(splatProgram.uniforms.point, e, Math.abs(r + window.scrollY / window.innerHeight) % 1) : gl.uniform2f(splatProgram.uniforms.point, e, r), gl.uniform3f(splatProgram.uniforms.color, t, n, 0), gl.uniform1f(splatProgram.uniforms.radius, correctRadius(config.SPLAT_RADIUS / 100)), blit(velocity.write), velocity.swap(), gl.uniform1i(splatProgram.uniforms.uTarget, dye.read.attach(0)), gl.uniform3f(splatProgram.uniforms.color, i.r, i.g, i.b), blit(dye.write), dye.swap()
}
...
function HSVtoRGB(e, r, t) {
    let n, i, o, a, l, u, c, g;
    switch (a = Math.floor(6 * e), l = 6 * e - a, u = t * (1 - r), c = t * (1 - l * r), g = t * (1 - (1 - l) * r), a % 6) {
        case 0:
            n = t, i = g, o = u;
            break;
        case 1:
...
update(), homePageBody.addEventListener("mouseover", (e => {
    let r = scaleByPixelRatio(e.clientX),
        t = scaleByPixelRatio(e.clientY),
    ...
    for (let e = 0; e < r.length; e++) {
        let t = pointers[e + 1];
        t.down && updatePointerMoveData(t, scaleByPixelRatio(r[e].pageX), scaleByPixelRatio(r[e].pageY))
    }
